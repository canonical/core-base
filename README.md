...
at `/snap/core22/current/usr/share/doc/ChangeLog`.

You can list all installed revisions with `snap list --all core22`. To inspect
a specific revision that is not the active one, replace `current` with the
revision number (e.g. `/snap/core22/1234/usr/share/doc/ChangeLog`).
...
unsquashfs -d core22-unpacked core22_*.snap usr/share/doc/ChangeLog
```

The ChangeLog is then available at `core22-unpacked/usr/share/doc/ChangeLog`.
